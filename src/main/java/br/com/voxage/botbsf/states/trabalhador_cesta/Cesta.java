					...
					try {
						...
						if(aux != null && aux == 1) {
							if("ENTREGUE".equals(cesta.getSituacao())) {
								...
								messages = SimpleMessage.text(String.format("Você já recebeu sua cesta de número %s em %s.\n"
										+ "Todas as cestas que você tinha como benefício já foram entregues.", cesta.getNumero().toString(), dia.format(date)));
								...
								Message<?> messages = null;
								messages = SimpleMessage.text(String.format("Você receberá sua cesta número %s até %s", cesta.getNumero().toString(), dia.format(date)));
								bot.addResponse(messages);
								botInputResult.setIntentName(BotBSF.STATES.FINALIZAR);
							}
						}else {
							...
							Message<?> messages = null;
							messages = SimpleMessage.text("Você não possui cestas de alimentos a serem entregues por este protocolo");
							bot.addResponse(messages);
							botInputResult.setIntentName(BotBSF.STATES.FINALIZAR);
						}
